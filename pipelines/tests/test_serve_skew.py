...
def test_serve_features_no_skew(con):
    ...
    for mid in _sample(con):
        teams = [r[0] for r in con.execute(
            "select distinct team_id from marts.mart_1n2 where match_id=? order by team_id", [mid]).fetchall()]
        if len(teams) != 2:
            continue
        A, B = teams
        xi = {t: [r[0] for r in con.execute(
            "select distinct player_id from intermediate.int_whoscored_lineup where match_id=? and team_id=? and start_minute=0",
            [mid, t]).fetchall()] for t in teams}
        gk = {t: con.execute("""select player_id from intermediate.int_whoscored_player_match
            where match_id=? and team_id=? and position='GK' and is_first_eleven
            qualify row_number() over(partition by match_id,team_id order by player_id)=1""", [mid, t]).fetchone() for t in teams}
        if any(g is None for g in gk.values()):
            continue
        compo = {t: {"xi": xi[t], "gk": gk[t][0]} for t in teams}
        xi_pos = con.execute(
            "select team_id,player_id,grid_horizontal from intermediate.int_whoscored_lineup where match_id=? and start_minute=0",
            [mid]).fetchall()

        zonal = sf.zonal_features(con, mid, xi_pos)
        for t, opp in [(A, B), (B, A)]:
            season = con.execute("select any_value(season) from marts.mart_1n2 where match_id=? and team_id=?", [mid, t]).fetchone()[0]
            feats = sf.match_compo_features(con, mid, t, opp, compo, season)
            feats.update(zonal.get(t, {}))
            stored = con.execute(f"select {', '.join(feats.keys())} from marts.mart_1n2 where match_id=? and team_id=?",
                                 [mid, t]).fetchdf().iloc[0].to_dict()
            for k in feats:
