...
=========================================================
Ex-03b_features_match_details.py, déplacé dans le package features/.

Transforme les événements bruts silver.stg_whoscored_events en features
spatiales et comportementales (Blocs A–G + has_ws_events).

ARCHITECTURE 3 PASSES
──────────────────────
  Passe 1 — Explosion qualifiers_json (UNNEST DuckDB)
  Passe 2 — Agrégations spatiales par (ws_match_id, team_id)
  Passe 3 — Pivot home/away + anti-leakage LAG(1) + UPDATE features_training
...
from .columns import NEW_COLS_WS, DIFF_COLS_WS
...
WHERE e.qualifiers_json IS NOT NULL AND e.qualifiers_json != '[]'
;
"""
...
SELECT
    ws_match_id, team_id, minute, second, expanded_minute, period,
    x, y, end_x, end_y, type_id, type_name, outcome_id,
...
defensive_exposure AS (
    ...
    FROM (
        ...
        JOIN (SELECT ws_match_id, team_id FROM tmp_events_flat GROUP BY ws_match_id, team_id) other
...
qualifier_features AS (
    SELECT ws_match_id, team_id,
        ...
        COUNT(*) FILTER (WHERE qual_type_id=155 AND outcome_id=1)                        AS through_balls_successful,
...
momentum_windows AS (
    ...
    JOIN tmp_events_flat e ON e.ws_match_id=gc.ws_match_id
...
counter_attack_cte AS (
    ...
    FROM (
        ...
        FROM tmp_events_flat WHERE type_id IN (13, 14, 15, 16)
    ) s
    LEFT JOIN (
        ...
        JOIN (
            ...
            FROM tmp_events_flat WHERE type_id = 49
        ) r ON s2.ws_match_id = r.ws_match_id AND s2.team_id = r.team_id
           AND s2.t_shot > r.t_recovery AND s2.t_shot - r.t_recovery <= 15
    ) t ON s.ws_match_id = t.ws_match_id AND s.team_id = t.team_id
       AND s.t_shot = t.t_shot
    GROUP BY s.ws_match_id, s.team_id
),
midfield_control_cte AS (
    SELECT
        f.ws_match_id, f.team_id,
        CAST(COUNT(*) FILTER (WHERE f.x BETWEEN 33 AND 66
                                AND f.type_id IN (1, 7, 8) AND f.outcome_id = 1) AS DOUBLE)
        / NULLIF(mt.match_midfield_total, 0) AS ws_midfield_control_idx
    FROM tmp_events_flat f
    JOIN (
        SELECT ws_match_id,
               SUM(COUNT(*) FILTER (WHERE x BETWEEN 33 AND 66
                                     AND type_id IN (1, 7, 8) AND outcome_id = 1))
               OVER (PARTITION BY ws_match_id) AS match_midfield_total
        FROM tmp_events_flat GROUP BY ws_match_id, team_id
    ) mt ON f.ws_match_id = mt.ws_match_id
    GROUP BY f.ws_match_id, f.team_id, mt.match_midfield_total
),
defensive_shape_cte AS (
    SELECT
        ws_match_id, team_id,
        AVG(x) AS ws_defensive_line_height,
        -- Asymétrie flancs : positif = meilleur à droite, négatif = meilleur à gauche
        CASE WHEN COUNT(*) FILTER (WHERE y < 30) > 0
...
assembled AS (
    SELECT
        ...
        CASE WHEN ca.total_shots_ca > 0
             THEN CAST(ca.counter_attack_shots AS DOUBLE) / ca.total_shots_ca
             ELSE NULL END                                         AS ws_counter_attack_dna,
...
home_features AS (
    SELECT f.ws_match_id,
        ...
        f.ws_counter_attack_dna    AS home_counter_attack_dna,
        f.ws_midfield_control_idx  AS home_midfield_control_idx,
        f.ws_defensive_line_height AS home_defensive_line_height,
        f.ws_flank_exposure_asymm  AS home_flank_exposure_asymm
    FROM tmp_team_features f JOIN match_meta m ON f.ws_match_id=m.ws_match_id AND f.team_id=m.home_team_id
),
away_features AS (
    SELECT f.ws_match_id,
        ...
        f.ws_counter_attack_dna    AS away_counter_attack_dna,
        f.ws_midfield_control_idx  AS away_midfield_control_idx,
        f.ws_defensive_line_height AS away_defensive_line_height,
        f.ws_flank_exposure_asymm  AS away_flank_exposure_asymm
    FROM tmp_team_features f JOIN match_meta m ON f.ws_match_id=m.ws_match_id AND f.team_id=m.away_team_id
)
SELECT
    m.ws_match_id, m.season, mi.match_date, mi.home_team_name, mi.away_team_name,
...
home_side AS (
    SELECT
        p.match_date AS ws_date, p.season AS ws_season,
...
away_side AS (
    SELECT
        p.match_date, p.season,
        COALESCE(tm.canonical_name, p.away_team_name) AS team_name,
        p.away_field_tilt_actions, p.away_high_turnover_rate,
        p.away_deep_completion_rt, p.away_momentum_delta,
        p.away_counter_shot_rate, p.away_set_piece_pressure,
        p.away_attack_left_pct, p.away_attack_center_pct, p.away_attack_right_pct,
        p.away_zone_def_pct, p.away_zone_mid_pct, p.away_zone_att_pct,
        p.away_shot_six_yard_pct, p.away_shot_penalty_pct, p.away_shot_oob_pct,
        p.away_shot_open_play_pct, p.away_shot_set_piece_pct, p.away_shot_penalty_att_pct,
        p.away_conversion_rate, p.away_cross_rate, p.away_through_ball_rate,
        p.away_long_ball_rate, p.away_short_pass_rate,
...
def add_columns_if_not_exist(conn: duckdb.DuckDBPyConnection) -> None:
    for col_name, col_type in NEW_COLS_WS:
        try:
    ...
    for col_name, col_type in DIFF_COLS_WS:
        try:
...
def reset_columns(conn: duckdb.DuckDBPyConnection) -> None:
    logger.warning("  Reset des colonnes ws_* → NULL...")
    for col_name, _ in NEW_COLS_WS:
        try:
            conn.execute(f"UPDATE gold.features_training SET {col_name}=NULL")
        except Exception:
            pass
    for col_name, _ in DIFF_COLS_WS:
        try:
...
def inject_team_mapping(conn: duckdb.DuckDBPyConnection) -> None:
    if not TEAM_MAPPING_ROWS:
        conn.execute("""
            CREATE OR REPLACE TEMP TABLE tmp_team_mapping (raw_name VARCHAR, canonical_name VARCHAR)
        """)
    ...
    conn.execute("""
        ...
        SELECT raw_name, canonical_name FROM _df_team_mapping
    """)
...
def run_passe1_2(conn: duckdb.DuckDBPyConnection) -> int:
    logger.info("Passe 1 — Explosion qualifiers_json (UNNEST)...")
    conn.execute(SQL_EVENTS_FLAT)
    n_flat = conn.execute("SELECT COUNT(*) FROM tmp_events_flat").fetchone()[0]
    logger.info(f"  {n_flat:,} événements dans tmp_events_flat")
    ...
    logger.info("Passe 2 — Agrégations spatiales...")
...
def run_passe3(conn: duckdb.DuckDBPyConnection) -> int:
    ...
    logger.info("  UPDATE gold.features_training avec LAG(1)...")
    conn.execute("""
        CREATE OR REPLACE TEMP TABLE tmp_latest_ws AS
        SELECT ft.team AS team, ft.date AS ft_date, wsh.ws_date,
            wsh.ws_field_tilt_actions, wsh.ws_high_turnover_rate,
            wsh.ws_deep_completion_rt, wsh.ws_momentum_delta,
            wsh.ws_counter_shot_rate, wsh.ws_set_piece_pressure,
            ...
            wsh.ws_counter_attack_dna,
            wsh.ws_midfield_control_idx,
            wsh.ws_defensive_line_height,
            wsh.ws_flank_exposure_asymm
        ...
        QUALIFY ROW_NUMBER() OVER (PARTITION BY ft.team, ft.date ORDER BY wsh.ws_date DESC) = 1
    """)
    ...
    conn.execute("""
        UPDATE gold.features_training AS ft SET
            ...
            ws_counter_attack_dna    = lws.ws_counter_attack_dna,
            ws_midfield_control_idx  = lws.ws_midfield_control_idx,
            ws_defensive_line_height = lws.ws_defensive_line_height,
            ws_flank_exposure_asymm  = lws.ws_flank_exposure_asymm
        FROM tmp_latest_ws lws
        WHERE ft.team=lws.team AND ft.date=lws.ft_date
    """)
    ...
    n_updated = conn.execute(
        "SELECT COUNT(*) FROM gold.features_training WHERE ws_field_tilt_actions IS NOT NULL"
    ).fetchone()[0]
...
def build_differential_features(conn: duckdb.DuckDBPyConnection) -> None:
    logger.info("  Calcul différentiels ws_* dans gold.features_final...")
    conn.execute("""
...
if __name__ == "__main__":
    ...
    run_pipeline(reset_cols=args.reset_cols, coverage_only=args.coverage_only)
