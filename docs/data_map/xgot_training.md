...
Dataset d'entraînement du modèle xGOT (post-shot xG) = xgot_features + label. Un tir cadré éligible par ligne. Cible = label (le tir finit-il au fond). Toute la logique de features et le périmètre vivent dans xgot_features (partagé avec le scoring → pas de train/serve skew). Pas de filtre saison (protection portée par une barrière de couverture avant l'entraînement). Vue → toujours à jour. Schéma : machine_learning.
...
| `season` | VARCHAR | 100.0% | 0 | 8 | top: 2017-2018 (21340), 2018-2019 (17578), 2023-2024 (17447) | Saison — sert au split temporel et au suivi de couverture (pas au filtrage). |
...
| `label` | INTEGER | 100.0% | 0 | 2 | moy 0.298 · méd 0.0 · min/max 0.0/1.0 · p10/p90 0.0/1.0 | Cible binaire : 1 = but, 0 = tir cadré arrêté. |