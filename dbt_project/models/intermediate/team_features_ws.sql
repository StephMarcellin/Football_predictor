...
defensive_exposure AS (
    SELECT match_id, other_team_id AS team_id,
    ...
    FROM (
        SELECT f.match_id, f.x, f.y, f.is_touch, other.team_id AS other_team_id
        FROM {{ ref('int_whoscored_events') }} f
        JOIN (
            SELECT match_id, team_id
            FROM {{ ref('int_whoscored_events') }}
            WHERE match_id IN (SELECT match_id FROM new_match_ids)
            GROUP BY match_id, team_id
        ) other ON f.match_id=other.match_id AND f.team_id!=other.team_id
        WHERE f.is_touch=TRUE AND f.x>33
          AND f.match_id IN (SELECT match_id FROM new_match_ids)
    ) att_actions
    GROUP BY match_id, other_team_id
),
...
goals_conceded AS (
    SELECT DISTINCT f.match_id, other.team_id AS conceding_team_id,
...
momentum_windows AS (
    ...
    JOIN {{ ref('int_whoscored_events') }} e
        ON e.match_id = gc.match_id
...
counter_attack_cte AS (
    ...
    FROM (
        SELECT match_id, team_id, expanded_minute*60+second AS t_shot
        FROM {{ ref('int_whoscored_events') }}
        WHERE type_id IN (13,14,15,16)
          AND match_id IN (SELECT match_id FROM new_match_ids)
    ) s
    LEFT JOIN (
        SELECT DISTINCT s2.match_id, s2.team_id, s2.t_shot
        FROM (
            SELECT match_id, team_id, expanded_minute*60+second AS t_shot
            FROM {{ ref('int_whoscored_events') }}
            WHERE type_id IN (13,14,15,16)
        ...
        JOIN (
            SELECT match_id, team_id, expanded_minute*60+second AS t_recovery
            FROM {{ ref('int_whoscored_events') }}
            WHERE type_id=49
              AND match_id IN (SELECT match_id FROM new_match_ids)
        ) r ON s2.match_id=r.match_id AND s2.team_id=r.team_id
           AND s2.t_shot > r.t_recovery AND s2.t_shot - r.t_recovery <= 15
    ) t ON s.match_id=t.match_id AND s.team_id=t.team_id AND s.t_shot=t.t_shot
...
midfield_control_cte AS (
    SELECT match_id, team_id,
        ...
        / NULLIF(SUM(COUNT(*) FILTER (
            WHERE x BETWEEN 33 AND 66 AND type_id IN (1,7,8) AND outcome_id=1
        )) OVER (PARTITION BY match_id), 0) AS ws_midfield_control_idx
    FROM {{ ref('int_whoscored_events') }}
    WHERE match_id IN (SELECT match_id FROM new_match_ids)
    GROUP BY match_id, team_id
),
...
LEFT JOIN counter_attack_cte  ca ON b.match_id=ca.match_id AND b.team_id=ca.team_id
LEFT JOIN midfield_control_cte mc ON b.match_id=mc.match_id AND b.team_id=mc.team_id
LEFT JOIN defensive_shape_cte ds ON b.match_id=ds.match_id AND b.team_id=ds.team_id
