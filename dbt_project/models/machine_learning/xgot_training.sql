...
-- Cible : is_goal (le tir cadré finit-il au fond, sachant son placement).
-- Grain : un tir cadré éligible. Matérialisé en VUE → toujours à jour avec
-- int_shot_placement (utile tant que le re-scrape complète la couverture).
--
-- Filtres (voir ADR / Concepts) :
--   • is_on_target        : xGOT n'existe que pour un tir cadré (SavedShot + Goal)
--   • goal_mouth_y NOT NULL: validité — sans placement, pas de features
--   • hors CSC            : un but contre son camp n'est pas un tir à modéliser
--   • hors penalty (qual 9): régime de conversion à part (~79 %), traité séparément
--
-- IMPORTANT — pas de filtre sur la saison : le dataset est défini « plein ».
-- La protection contre l'entraînement sur données incomplètes/biaisées est portée
-- par une BARRIÈRE DE COUVERTURE exécutée avant le train (script/pipeline), pas par
-- un filtre figé ici.
--
