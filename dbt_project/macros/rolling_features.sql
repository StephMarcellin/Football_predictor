{% macro rolling_cols(w) %}
    ...
    {% set fv %}PARTITION BY team_id, season, league_source, venue ORDER BY date ROWS BETWEEN {{ w }} PRECEDING AND 1 PRECEDING{% endset %}

    -- Moyenne des buts marqués et encaissés sur les {{ w }} derniers matchs
    AVG(gf)                       OVER ({{ fg }}) / NULLIF(AVG(np_xg) OVER ({{ fg }}), 0) - 1 AS xg_overperformance_{{ w }},
    AVG(ga)                       OVER ({{ fg }}) / NULLIF(AVG(np_xg_conceded) OVER ({{ fg }}), 0) - 1 AS xg_conceded_overperformance_{{ w }},
    AVG(gf)                       OVER ({{ fg }}) AS avg_gf_{{ w }},
    AVG(ga)                       OVER ({{ fg }}) AS avg_ga_{{ w }},

    -- Moyenne des xG et xG concédés sur les {{ w }} derniers matchs
    ...
    -- Moyenne des tirs, tirs cadrés et tirs cadrés subis sur les {{ w }} derniers matchs
    AVG(shots_total)               OVER ({{ fg }}) AS avg_shots_{{ w }},
    AVG(shots_on_target)           OVER ({{ fg }}) AS avg_sot_{{ w }},
    AVG(clean_sheet)               OVER ({{ fg }}) AS cs_rate_{{ w }},
    ...
    AVG(saves)                    OVER ({{ fg }}) AS avg_saves_{{ w }},
